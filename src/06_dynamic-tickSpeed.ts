import {Counter, CountDownState, CounterStateKeys, PartialCountDownState} from './counter'
...
const tickSpeed$ = counterState$.pipe(pluck(CounterStateKeys.tickSpeed), distinctUntilChanged<number>());
const countDiff$ = counterState$.pipe(pluck(CounterStateKeys.countDiff), distinctUntilChanged<number>());
...
const commandFromTick$ = counterUpdateTrigger$
  .pipe(
     ...
     tap(({count, countUp}) => programmaticCommandSubject.next({count: count + 1 * (countUp ? 1 : -1)}))
  );
